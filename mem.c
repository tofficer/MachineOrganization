...
void* Mem_Alloc(int size)
{
  ...
  int reqSize = size + sizeof(block_header);

  block_header* curr = list_head;
  block_header*  best = NULL;

  //iterate through list to find the best free block ie free, bigger than the required size,                                                                                   
  //and less than the current best size                                                 
  while(curr != NULL){
    if (!(curr->size_status & 1) && curr->size_status >= reqSize
        && (best == NULL || curr->size_status < best->size_status) ){
      best = curr;
      if (best->size_status == reqSize) break;
 }
    curr = curr->next;
  }
  //if a suitable free block was found set as busy and return                           
  if (best != NULL){
    //if there the free block is not a perfect fit update the remaining block           
    int remaining = (best->size_status) - reqSize;
    if (remaining > 0){
      block_header* temp = (block_header*)((char*)best + reqSize);
      temp->size_status = remaining;
      temp->next = best->next;
      best->next = temp;
    }
    best->size_status =size +1;
    return (void*)((char*)best + sizeof(block_header));
  }
  return NULL;
}
...
{
  //make sure ptr isn't null                                                            
  if(ptr == NULL) return -1;
  //make sure ptr is pointing to first byte of busy block                               
  if(!((block_header*) (ptr - sizeof(block_header)))->size_status & 1) return -1;

  block_header* curr =  list_head;
  block_header* prev = NULL;

  //iterate through the list until you find the address of the block to be freed up     
  while(curr != NULL){
    if((void*)((char*)curr + sizeof(block_header)) == ptr){
      //set it to free                                                                  
      curr->size_status -= 1;
      break;
    }
...
//coalesce neighbors if free                                                          
  if(curr->next != NULL && (!(curr->next->size_status & 1))){
    curr->size_status += sizeof(block_header) + curr->next->size_status;
    curr->next = curr->next->next;
  }

  if(prev != NULL && (!(prev->size_status & 1))){
    prev->size_status += sizeof(block_header) + curr->size_status;
    prev->next = curr->next;
  }
...
/* Prints out a list of all the blocks along with the following information for each bl\
ock */
...
free_size = free_size + t_Size;
    ...
    End = Begin + Size;
    fprintf(stdout,"%d\t%s\t0x%08lx\t0x%08lx\t%d\t%d\t0x%08lx\n",counter,status,(unsign\
ed long int)Begin,(unsigned long int)End,Size,t_Size,(unsigned long int)t_Begin);
